...
const MyCourse = () => {
    ...
    return (
      <UserLayout>
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 transform">
          <div className="w-10 h-10 border-4 border-purple-600 rounded-full animate-spin border-t-transparent"></div>
        </div>
      </UserLayout>
    );
  }
  ...
  if (error) {
    return (
      <UserLayout>
        <div className="text-red-500 p-4 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 transform whitespace-nowrap ">
      ...
      <section className="sm:flex sm:flex-col lg:flex sm:space-y-8 sm:px-3 sm:py-5 sm:pb-24 ">
        {courses && courses.length > 0 ? (
          courses.map((enrolledCourse) => {
            const courseData = enrolledCourse.course;
