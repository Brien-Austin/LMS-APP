...
const InstructorLogin = ({ setAuthState }) => {
    ...
    const onSubmit = async (values) => {
        try {
            const response = await instructorApiClient.post(INSTRUCTOR_LOGIN, {
                email: values.email,
    ...
    return (
        <section className="flex-1">
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col w-4/5 lg:px-10 sm:px-2 mx-auto space-y-2">
                ...
                <button type="submit" className={cn("bg-gradient-to-r from-purple-500 to-purple-600 px-2 py-3 rounded-md text-white text-lg")}>
                    Login
                </button>

                <p className="text-center mt-2">
                    New user?{" "}
                    <span onClick={handleRegister} className="underline decoration-purple-600 text-purple-600">
                        Register Here
                    </span>
                </p>
            </form>
